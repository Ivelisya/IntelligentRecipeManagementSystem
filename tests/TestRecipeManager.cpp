   ...
   protected:
    ...
    void SetUp() override {
        ...
        std::string unique_name =
            "test_manager_dir_" + std::to_string(value.count());

        tempTestDir = std::filesystem::current_path() / unique_name;
        std::filesystem::create_directories(tempTestDir);

        repo = std::make_unique<RecipeApp::Persistence::JsonRecipeRepository>(
            tempTestDir, testDbFileName);
...
TEST_F(RecipeManagerTest, AddRecipeWithTags) {
    RecipeApp::Recipe recipe1 = createRecipeWithTags(
        0, "Pasta Carbonara", {"Italian", "Dinner", "Quick"});
    int id1 = manager->addRecipe(recipe1);
    ASSERT_GT(id1, 0);

    std::optional<RecipeApp::Recipe> fetchedRecipeOpt =
        manager->findRecipeById(id1);
...
TEST_F(RecipeManagerTest, UpdateRecipeChangeTags) {
    RecipeApp::Recipe recipe1 =
        createRecipeWithTags(0, "Salad", {"Healthy", "Lunch"});
    int id1 = manager->addRecipe(recipe1);
    ASSERT_GT(id1, 0);
    ...
    std::optional<RecipeApp::Recipe> updatedRecipeOpt =
        manager->findRecipeById(id1);
...
TEST_F(RecipeManagerTest, UpdateRecipeClearAllTags) {
    RecipeApp::Recipe recipe1 =
        createRecipeWithTags(0, "Steak", {"Meat", "Dinner", "Grill"});
    int id1 = manager->addRecipe(recipe1);
    ASSERT_GT(id1, 0);
...
TEST_F(RecipeManagerTest, FindRecipesBySingleTag) {
    ...
    std::vector<RecipeApp::Recipe> foundByTag1 =
        manager->findRecipesByTag("Tag1");
    ASSERT_EQ(foundByTag1.size(), 2);
    // Check names or IDs if necessary

    std::vector<RecipeApp::Recipe> foundByTag3 =
        manager->findRecipesByTag("Tag3");
    ASSERT_EQ(foundByTag3.size(), 1);
    EXPECT_EQ(foundByTag3[0].getName(), "Recipe B");

    std::vector<RecipeApp::Recipe> foundByNonExistentTag =
        manager->findRecipesByTag("NonExistent");
    ASSERT_TRUE(foundByNonExistentTag.empty());
}

TEST_F(RecipeManagerTest, FindRecipesByMultipleTags_MatchAll) {
    manager->addRecipe(createRecipeWithTags(
        0, "Recipe Alpha", {"Common", "AlphaFeature", "Primary"}));
    manager->addRecipe(createRecipeWithTags(
        0, "Recipe Beta", {"Common", "BetaFeature", "Primary"}));
    manager->addRecipe(createRecipeWithTags(
        0, "Recipe Gamma", {"Common", "AlphaFeature"}));  // Missing Primary
    manager->addRecipe(createRecipeWithTags(
        0, "Recipe Delta", {"AlphaFeature", "Primary"}));  // Missing Common

    std::vector<std::string> searchTags1 = {"Common", "AlphaFeature",
                                            "Primary"};
    std::vector<RecipeApp::Recipe> found1 =
        manager->findRecipesByTags(searchTags1, true);
    ASSERT_EQ(found1.size(), 1);
    EXPECT_EQ(found1[0].getName(), "Recipe Alpha");

    std::vector<std::string> searchTags2 = {"Common", "Primary"};
    std::vector<RecipeApp::Recipe> found2 =
        manager->findRecipesByTags(searchTags2, true);
    ASSERT_EQ(found2.size(), 2);  // Alpha and Beta

    std::vector<std::string> searchTags3 = {"AlphaFeature"};
    std::vector<RecipeApp::Recipe> found3 =
        manager->findRecipesByTags(searchTags3, true);
    ASSERT_EQ(found3.size(), 3);  // Alpha, Gamma, Delta

    std::vector<std::string> searchTagsNone = {"NonExistentTag", "Common"};
    std::vector<RecipeApp::Recipe> foundNone =
        manager->findRecipesByTags(searchTagsNone, true);
    ASSERT_TRUE(foundNone.empty());
}

TEST_F(RecipeManagerTest, FindRecipesByMultipleTags_MatchAny) {
    // Note: findRecipesByTags currently only supports matchAll=true based on
    // RecipeManager.h If matchAll=false (OR logic) is implemented, this test
    // would be valid. For now, this test will behave like matchAll=true or
    // needs adjustment if the method signature changes.
    manager->addRecipe(
        createRecipeWithTags(0, "Recipe X", {"UniqueX", "Shared1"}));
    manager->addRecipe(
        createRecipeWithTags(0, "Recipe Y", {"UniqueY", "Shared2"}));
    manager->addRecipe(
        createRecipeWithTags(0, "Recipe Z", {"Shared1", "Shared2"}));
    manager->addRecipe(createRecipeWithTags(0, "Recipe W", {"UniqueW"}));

    // Assuming current implementation (matchAll = true by default or only
    // option)
    std::vector<std::string> searchTagsOr = {"UniqueX", "UniqueY"};
    std::vector<RecipeApp::Recipe> foundOr =
        manager->findRecipesByTags(searchTagsOr, true);  // This will be AND
    ASSERT_TRUE(foundOr.empty());

    // If matchAll = false was an option:
    // std::vector<RecipeApp::Recipe> foundOr =
    // manager->findRecipesByTags(searchTagsOr, false);
